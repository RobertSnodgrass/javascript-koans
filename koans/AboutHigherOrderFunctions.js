...
describe("About Higher Order Functions", function() {
  ...
  it("should use range to generate an array", function() {
    expect(_.range(3)).toEqual([0, 1, 2]);//Only one value = starting point is 0; Goes from 0 - 3 but does not include 3 (uninclusive)
    expect(_.range(1, 4)).toEqual([1, 2, 3]);//Two values = starting point is 1, 4 is upper limit, uninclusive, so 3
    expect(_.range(0, -4, -1)).toEqual([0, -1, -2, -3], [0]);//Start with 0, -4 is lower end limit, uninclusive, so -3; second array is 0 to -1 uninclusive, so only one value
  });
  ...
  it("should use chain() ... .value() to use multiple higher order functions", function() {
    var result = _([ [0, 1], 2 ]).chain()
                     .flatten()//combine both arrays into [0, 1, 2]
                     .map(function(x) { return x+1 } )//maps +1 to all values; [1, 2, 3]
                     .reduce(function(sum, x) { return sum + x })//Incudes 0 , 1 -> reduce to 1; 1, 2 -> reduce to 3; 3, 3 -> reduce to 6
                     .value();//Gets value of array -> 6
